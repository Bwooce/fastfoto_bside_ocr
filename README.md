...
The system uses a **two-step workflow**: preprocessing standalone script + Claude Code interactive session.
...
1. **Preprocessing** (standalone script) - Prepares images for analysis
2. **Analysis** (Claude Code session) - Interactive OCR and EXIF updates
...
✅ **Format-agnostic** - Flexible date parsing trusts Claude's understanding
✅ **Rich EXIF support** - Writes EXIF, IPTC, and XMP fields (compatible with Lightroom, Photos, Google Photos)
...
### Step 1: Preprocessing (Command Line)

Prepare your FastFoto back scans for Claude Code analysis:
...
**What this does:**
- Finds all `*_b.jpg` files in your directory
- Resizes images >3.5MB or >2000px to 1800px @ 85% quality
- Converts TIFF to JPEG format
- Maintains directory structure in output folder
- Creates `preprocessing_mapping.json` for tracking
...
Start Claude Code and begin the interactive session:
...
**Claude Code will:**
1. Load the mapping file to understand original → prepared image relationships
2. Analyze each prepared image using the Read tool with OCR prompt
3. Extract metadata (dates, locations, text) from each image
4. Generate a proposal file showing all proposed EXIF updates
5. Display statistics and next steps

**Example session:**
```
Claude: I'll analyze the prepared FastFoto back scans. Let me load the mapping file first...

[Loads preprocessing_mapping.json to get original file paths]

Found 482 prepared images. I'll analyze each using the Read tool with the OCR prompt.
...
Please review the file and make any edits. When ready, say "apply the proposal"
...
```txt
  PROPOSED UPDATES:
    DateTimeOriginal: 1998:06:07 11:32:00  ← Changed from 1999 to 1998
```

### Step 4: Apply Updates (Claude Code Session)

After reviewing and editing the proposal, continue the Claude Code session:
...
├── Uses file_discovery.py to find *_b.jpg files
├── Uses image_processor.py to resize/convert images
└── Creates preprocessing_mapping.json

Step 2: Analysis (Claude Code Interactive)
Claude Code Session
├── Uses Read tool on prepared images
├── Uses interactive_processor.py helper
├── Generates proposal via proposal_generator.py
└── Applies updates via exif_writer.py
```

**Core Modules:**
```
src/
├── preprocess_images.py     # NEW: Standalone preprocessing script
├── interactive_processor.py # NEW: Helper for Claude Code sessions
├── file_discovery.py        # Find and pair _b files with originals
├── image_processor.py       # Resize images for Read tool constraints
├── claude_prompts.py        # Prompt templates and response parsing
├── date_parser.py           # Flexible date parsing (Spanish/English)
├── exif_writer.py          # ExifTool wrapper for metadata writing
├── proposal_generator.py    # Generate human-reviewable proposal files
└── orchestrator.py          # Legacy - may be removed
```
...
**Benefits:**
- **Separation of concerns**: Image processing vs. AI analysis
- **Testable**: Verify preprocessing worked before analysis
- **Reusable**: Can rerun analysis on same prepared images
- **Efficient**: No waiting for resizing during interactive session
...
- ✅ No separate API costs (uses Claude Max)
- ✅ Interactive workflow with user control
- ✅ Built-in image reading capabilities
- ❌ Requires active session (not standalone)
...
- Contextual understanding ("27 de Noviembre de 1983" → date)
...
**Common issues:**
- Two-digit years: 02 → 2002 or 1902?
