...
- Size reduction: 68.7% (2847MB → 892MB)
...
```
Claude Code Session
├── Runs preprocessing via Bash tool (src/preprocess_images.py)
...
├── Uses Read tool on each prepared image
├── Uses interactive_processor.py helper for coordination
├── Generates proposal via proposal_generator.py
└── Applies updates via exif_writer.py (when requested)
```
